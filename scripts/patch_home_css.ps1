...
# patch_home_css.ps1 — replaces the big inline CSS block in Smart_Picks_Pro_Home.py
# with the new PrizePicks + DraftKings Pick 6 AI theme.
...
# The inline CSS block starts with st.markdown("""  (at the second occurrence — right after get_global_css())
# and ends with """, unsafe_allow_html=True) at line 1235.
# We find the opening <style> and its matching </style>.

$styleStart = $raw.IndexOf('<style>' + "`n/* ═══════════════════════════════════════════════════════════`n   LANDING PAGE")
if ($styleStart -lt 0) {
    # fallback — find <style> inside the second st.markdown block
    $first = $raw.IndexOf('<style>')
    $styleStart = $raw.IndexOf('<style>', $first + 1)
}
$styleEnd = $raw.IndexOf('</style>', $styleStart) + '</style>'.Length

Write-Host "Home CSS block: $styleStart -> $styleEnd (len $($styleEnd - $styleStart))"
...
/* ── Landing Page Animation helpers ──────────────────────── */
...
@keyframes lpCardShine { 0%{ left:-100%; } 100%{ left:200%; } }
...
/* Hero Tagline — Inter bold gradient instead of Orbitron */
.hero-tagline {
...
/* ── Status Card (override global for landing) ────────────── */
.status-card {
    background: #161B27;
...
.pillar-title {
    font-size: 1.0rem; font-weight: 800; color: #FFFFFF;
    font-family: 'Inter', sans-serif;
}
...
.pipeline-step-num {
    font-family: 'Bebas Neue', 'Inter', sans-serif;
    font-size: 1.6rem; font-weight: 900; color: #00D559;
...
.nav-card-icon {
    ...
    filter: drop-shadow(0 0 8px rgba(0,213,89,0.25));
}
...
.comp-table th {
    padding: 10px 14px; font-size: 0.70rem; font-weight: 700;
    color: #6B7A9A; text-transform: uppercase; letter-spacing: 1px;
    background: #0D0F14; border-bottom: 1px solid rgba(255,255,255,0.07);
    text-align: left;
}
.comp-table td {
    padding: 10px 14px; font-size: 0.84rem; color: #E0E8FF;
...
/* ── LP Footer ────────────────────────────────────────────── */
.lp-footer {
    text-align: center; font-size: 0.72rem; color: #6B7A9A;
    padding: 28px 0 14px;
...
$before = $raw.Substring(0, $styleStart)
$after  = $raw.Substring($styleEnd)
$result = $before + $newCss + $after

[System.IO.File]::WriteAllText($file, $result, [System.Text.Encoding]::UTF8)
Write-Host "Done. New home file length: $($result.Length)"
